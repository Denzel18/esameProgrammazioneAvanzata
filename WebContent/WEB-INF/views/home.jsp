 ...
 <div class="container">
    <div class="row">
      ...
      <div>
		  ...
		  <h2 class="my-4 text-center">${CarsTitle}</h2>
		  <c:choose>
			  <c:when test="${numCars != 0}">
				  <div class="font-weight-bold text-center">Numero Veicoli: ${numCars}</div>
			  </c:when>
			  <c:otherwise>
				  <div class="font-weight-bold text-center">Nessun veicolo presente!</div>
			  </c:otherwise>
		  </c:choose>
		...
		<div>
			<c:forEach items="${cars}" var="c">
				...
				<div class="card-body">
					<h2 class="card-title">${c.marca}</h2>
					<p class="card-text">${c.modello}</p>
					 <a href="<c:url value="/car/${c.id}"/>" class="btn btn-primary">Dettagli veicolo &rarr;</a>
				</div>
			</c:forEach>
		</div>
